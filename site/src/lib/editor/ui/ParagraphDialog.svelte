<script lang="ts">
  ...
  import { cm, getParagraphPropertiesEffective, setParagraphAlignment, setParagraphIndent, setParagraphSpacing, setParagraphLineSpacing, type ParagraphProperties } from '@office-kit/pptx';
  ...
  function submit(event: SubmitEvent) {
    ...
    const changed = alignment !== original.alignment || left !== original.left || special !== original.special || by !== original.by || before !== original.before || after !== original.after || line !== original.line || at !== original.at;
...
<dialog bind:this={dialog} aria-label={t('Paragraph')} {onclose} onkeydown={event => event.stopPropagation()}>
  <form onsubmit={submit}>
    <h2>{t('Paragraph')}</h2>
    <div class="tab">{t('Indents and Spacing')}</div>
...
<style>
  ...
  .tab { text-align: center; margin: 14px auto; width: max-content; background: #626262; padding: 4px 12px; border-radius: 4px; }
