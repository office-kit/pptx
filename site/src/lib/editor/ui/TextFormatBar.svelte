<script lang="ts">
  ...
  let { formats, selected, onformat, ontoggle, ondone, onlink, paragraph, onparagraph, context = 'text' }: {
    ...
    ontoggle?: (property: 'bold' | 'italic' | 'underline') => void;
    ondone?: () => void;
    onlink?: () => void;
...
<div class="text-format-bar" role="group" aria-label={t(context === 'cells' ? 'Format selected cells' : 'Selected text formatting')}>
  ...
  <button class="ok-btn" aria-label={t('Bold')} aria-pressed={bold} disabled={!selected} onmousedown={(e) => e.preventDefault()} onclick={() => ontoggle ? ontoggle('bold') : onformat({bold: !bold})}><b>B</b></button>
  <button class="ok-btn" aria-label={t('Italic')} aria-pressed={italic} disabled={!selected} onmousedown={(e) => e.preventDefault()} onclick={() => ontoggle ? ontoggle('italic') : onformat({italic: !italic})}><i>I</i></button>
  <button class="ok-btn" aria-label={t('Underline')} aria-pressed={underline} disabled={!selected} onmousedown={(e) => e.preventDefault()} onclick={() => ontoggle ? ontoggle('underline') : onformat({underline: !underline})}><u>U</u></button>
