<script lang="ts">
  ...
  const stop = $derived(gradient?.stops[selectedIndex]);
  ...
  const track = $derived(gradient ? `linear-gradient(to right, ${[...gradient.stops].sort((a, b) => a.offset - b.offset).map(stop => `${stopColor(stop)} ${stop.offset * 100}%`).join(', ')})` : '');
...
{#if gradient && stop}
  <fieldset disabled={locked} class="gradient-fields">
    ...
    <div class="stops" role="group" aria-label={t('Gradient stops')} style:background={track}>
      {#each gradient.stops as item, index}
        <button type="button" class:selected={selectedIndex === index} class="stop" style:left={`${item.offset * 100}%`} style:background={stopColor(item)} aria-label={`${t('Gradient stop')} ${index + 1}`} aria-pressed={selectedIndex === index} title={`${Math.round(item.offset * 100)}%`} onclick={() => selected = index} onkeydown={event => {
...
<style>
  ...
  .stop { position: absolute; top: 10px; transform: translateX(-50%); width: 12px; height: 18px; border: 1px solid #555; border-radius: 2px; padding: 0; }
