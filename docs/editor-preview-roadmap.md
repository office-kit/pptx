...
The link command now opens a bilingual dialog with existing URL and tooltip values, mixed-selection feedback, batch application and removal, and a stale-selection guard. Browser coverage verifies cancellation, preserved bold formatting, undo/redo, and reload. Links on selected text ranges, images, and internal slide destinations still need dedicated editing workflows.

Internal slide-link targets now resolve against the current slide order even when link readers return reconstructed handles. Regression coverage checks preview SVG destinations, reorder/save/reload, deleted targets, and isolation between presentations. The development viewer now follows internal links in preview and presentation, supports Enter activation, and avoids ordinary click advancement when either internal or external links are activated. Browser coverage also verifies external links open separately and existing timers, skipped slides, and bilingual presenter controls still work. Editing internal destinations and link activation inside the separate presenter window remain pending.
