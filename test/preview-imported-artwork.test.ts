...
import {
  addSlide, addSlideImage, addSlideShape, findSlideLayout, getSlides, inches,
  loadPresentation, savePresentation, setShapeGradientFill,
...
const record = (type: number, values: number[] = []): Uint8Array => {
  ...
  [type, bytes.length, ...values].forEach((value, index) => view.setUint32(index * WORD_BYTES, value, true));
  return bytes;
};
...
const pathRecords = [
  record(17, [8]), record(9, [EMF_EXTENT, EMF_EXTENT]), record(11, [EMF_EXTENT, EMF_EXTENT]),
  record(39, [1, 0, 0x00ffffff, 0]), record(37, [1]), record(19, [2]),
  record(59), record(27, [10, 10]),
  record(6, [0, 0, 90, 90, 2, 90, 10, 90, 90]),
  record(5, [0, 0, 90, 90, 3, 80, 90, 20, 90, 10, 10]),
  record(61), record(60), record(62, [0, 0, 90, 90]),
];

const blankSlide = async () => {
  const pres = await loadPresentation(await readFile(new URL('./fixtures/minimal/blank.pptx', import.meta.url)));
  const layout = findSlideLayout(pres, 'Blank')!;
  return { pres, slide: addSlide(pres, { layout }) };
};
const loadMetafile = async (bytes: Uint8Array) => {
  const { pres, slide } = await blankSlide();
  addSlideImage(slide, buildPng(1, 1, [0, 0, 0]), { x: inches(0), y: inches(0), w: inches(1), h: inches(1) });
  const { entries } = readZip(await savePresentation(pres));
  const modified = entries.map((entry) => {
    if (entry.name.startsWith('ppt/media/')) return { ...entry, name: entry.name.replace('.png', '.emf'), data: bytes };
    if (entry.name.endsWith('.rels') || entry.name === '[Content_Types].xml') {
      return { ...entry, data: new TextEncoder().encode(new TextDecoder().decode(entry.data).replaceAll('.png', '.emf').replace('Extension="png" ContentType="image/png"', 'Extension="emf" ContentType="image/x-emf"')) };
    }
    return entry;
  });
...
describe('imported artwork', () => {
  it('位置が順不同のグラデーションを位置順に描画する', async () => {
    const { pres, slide } = await blankSlide();
    const shape = addSlideShape(slide, { preset: 'rect', x: inches(0), y: inches(0), w: inches(1), h: inches(1) });
    setShapeGradientFill(shape, { angleDeg: 90, stops: [
      { offset: 0.52, color: '#003C7A' }, { offset: 0, color: '#002060' }, { offset: 1, color: '#01A8DC' },
    ] });
  ...
  it('Node の PNG 出力にも EMF が描画され、パスの外側は透明になる', async () => {
    const redPaths = pathRecords.map((bytes) => new DataView(bytes.buffer).getUint32(0, true) === 39
      ? record(39, [1, 0, 0x000000ff, 0]) : bytes);
    const { pres, slide } = await loadMetafile(metafile(redPaths));
    const { image } = renderSlideToRgba(pres, slide, { width: 960 });
    const channels = 4;
    const pixel = (x: number, y: number) => [...image.data.subarray((y * image.width + x) * channels, (y * image.width + x + 1) * channels)];
  ...
  it('放射状グラデーションも位置順に並べてから反転する', async () => {
    const { pres, slide } = await blankSlide();
    const shape = addSlideShape(slide, { preset: 'rect', x: inches(0), y: inches(0), w: inches(1), h: inches(1) });
    setShapeGradientFill(shape, { path: 'circle', stops: [
      { offset: 0.52, color: '#003C7A' }, { offset: 0, color: '#002060' }, { offset: 1, color: '#01A8DC' },
    ] });
    const svg = renderSlideToSvg(pres, slide);
    expect([...svg.matchAll(/<stop offset="([^"]+)"/g)].map((match) => Number(match[1]))).toEqual([0, 0.48, 1]);
  });

  it('未対応の描画命令では部分描画せず代替表示を維持する', async () => {
    expect(await renderMetafile(metafile([...pathRecords, record(84)]))).toContain('data-pptx-fallback="image"');
  });
...
describe('EMF path decoding', () => {
  it('16 ビット座標の折れ線とベジェ曲線を符号付きで読み取る', () => {
    ...
    const bytes = metafile([
      record(59), record(27, [0, 0]),
      record(89, [0, 0, 90, 90, 1, packedPoint(-10, 20)]),
      record(88, [0, 0, 90, 90, 3, packedPoint(-20, 30), packedPoint(40, -50), packedPoint(60, 70)]),
      record(61), record(60), record(62, [0, 0, 90, 90]),
    ]);
  ...
  it('ウィンドウとビューポートの原点・倍率を適用してクリップする', () => {
    const bytes = metafile([
      record(17, [8]), record(10, [10, 20]), record(12, [5, 6]),
      record(9, [100, 200]), record(11, [200, -400]),
      record(59), record(27, [10, 20]), record(6, [0, 0, 100, 100, 2, 30, 20, 30, 40]),
      record(61), record(60), record(67, [5]),
      ...pathRecords,
      record(75, [0, 5]), ...pathRecords,
    ]);
  ...
  it.each([
    ...
    expect(renderEmfToSvg(metafile([
      record(59), record(27, [0, 0]), invalid, record(61), record(60), record(62, [0, 0, 90, 90]),
    ]))).toBeNull();
  });
  ...
  it('EMF+ コメント内の描画を無視して部分描画しない', () => {
    expect(renderEmfToSvg(metafile([...pathRecords, record(70, [WORD_BYTES, 0x2b464d45])]))).toBeNull();
  });
});
