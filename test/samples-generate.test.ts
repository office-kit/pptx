...
// Minimal hand-rolled PNG builder. Produces a single-color RGB image of the
// requested dimensions. Used to embed recognizable images in samples without
...
const buildPng = (
  ...
  // PNG IDAT is zlib-wrapped DEFLATE. Build zlib header + deflated body +
  // adler32 trailer.
  const deflated = deflateRawSync(raw);
  const adler = adler32(raw);
  const idatBody = new Uint8Array(2 + deflated.length + 4);
  idatBody[0] = 0x78; // zlib header
  idatBody[1] = 0x9c;
  idatBody.set(deflated, 2);
  const trailer = new DataView(idatBody.buffer, idatBody.byteOffset + 2 + deflated.length, 4);
  trailer.setUint32(0, adler >>> 0);
