...
describe('fn API: setChartSpec', () => {
  ...
  // The xy kinds carry their channels per series, so a write path that
  // forgot them would emit a chart with nothing to plot. Both entry points
  // reject that spec; the full xy round trip lives in fn-chart-kinds.test.ts.
  it('addSlideChart rejects an xy series without its x channel', async () => {
    const pres = await loadPresentation(await readFile(fixture('two-slides.pptx')));
    const slide = getSlides(pres)[0]!;
  ...
  it('setChartSpec switches an existing chart to scatter', async () => {
    const pres = await loadPresentation(await readFile(fixture('two-slides.pptx')));
    const slide = getSlides(pres)[0]!;
    addSlideChart(slide, {
      x: inches(0),
    ...
    const after = getSlideCharts(getSlides(reloaded)[0]!)[0]!;
    expect(after.spec?.kind).toBe('scatter');
    expect(after.spec?.series[0]?.xValues).toEqual([1, 2]);
  });
});
