...
//
//   1. HARD GATE — every slide @office-kit/pptx emits is schema-valid against the
...
//      count may only shrink; a change that pushes @office-kit/pptx further from the
...
// per-case report is either a @office-kit/pptx gap to close or a documented, accepted
...
// colors and vertical-center anchors where @office-kit/pptx inherits from the theme,
// and @office-kit/pptx marks real text boxes with `txBox="1"` where PptxGenJS does
...
describe.skipIf(!pptxGenJsAvailable())('PptxGenJS parity corpus', () => {
  for (const c of CASES) {
    it(c.id, async () => {
      ...
      // 1. Hard gate: @office-kit/pptx's slide must be schema-valid.
      ...
      if (r.kitChartSemantics || r.pgjsChartSemantics) {
        expect(
          r.kitChartSemantics,
          `${c.id}: @office-kit/pptx emitted no readable chart`,
        ).not.toBeNull();
