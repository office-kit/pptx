...
test(
  'random bars reveal horizontal and vertical bands in shuffled order',
  { timeout: 60000 },
  async () => {
    const dir = await mkdtemp(join(tmpdir(), 'office-random-bars-'));
    let preview, browser;
    try {
      const options = [
        { effect: 'none' },
        { effect: 'randomBar' },
        ...['horz', 'vert'].map((direction) => ({ effect: 'randomBar', direction })),
      ];
      ...
      for (let i = 1; i < options.length; i++) {
        ...
        for (const time of [0, 500, 1000]) {
            ...
            ({ options, time }) => {
              ...
              for (let row = 0; row < 16; row++)
                for (const fraction of [0.25, 0.75]) {
                  ...
                  const x = vertical ? (row + 0.5) / 16 : fraction;
                  const y = vertical ? fraction : (row + 0.5) / 16;
                  samples.push(
                    slide.shadowRoot
                      .elementFromPoint(
          ...
          assert.equal(
            result.samples.filter(Boolean).length,
            time === 0 ? 0 : time === 500 ? 16 : 32,
          );
          for (let row = 0; row < 16; row++)
            assert.equal(
              result.samples[row * 2],
              result.samples[row * 2 + 1],
              'each bar spans the slide',
            );
          if (time === 500) await page.screenshot({ path: `/tmp/pptx-random-bars-${i}.png` });
        }
