...
for (const flip of [
  ...
  test(
    `grouped text preserves preview orientation and scale in both languages (H=${flip.horizontal}, V=${flip.vertical})`,
    { timeout: 60000 },
    async () => {
      ...
      try {
        ...
        assert.ok(
          Math.abs(hitBounds.y + hitBounds.height / 2 - inputBounds.y - inputBounds.height / 2) < 1,
        );
