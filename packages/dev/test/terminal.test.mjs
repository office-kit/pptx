...
`,
    ...
    const terminal = createTerminal(join(directory, 'deck.tsx'), () => busy, '', async () => buildError);
    ...
    const verify = async () => (await fetch(hook.url, {
      method: 'POST', headers: hook.headers, body: '{}',
    })).json();
