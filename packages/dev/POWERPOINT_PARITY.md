...
- Preview focus coordinates now use edge insets, fixing corner directions that previously rendered at the center. Regression tests cover all four corners and the center, including the final-stop color at the focus.
...
- Native `tileRect` is read and written through the gradient API, with round-trip and schema tests; its general preview rendering remains outstanding. Rectangular and shape-following slide fills remain radial approximations. Type-switch defaults and remembered direction behavior still need parity verification: the audited Linear-to-Rectangular transition saved Bottom Right, whereas Rectangular Center-to-Path retained Center.
