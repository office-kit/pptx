...
- Native `tileRect` is read and written through the gradient API, with round-trip and schema tests; its general preview rendering remains outstanding. The native Rectangular and Path galleries still need comparison, and Type-switch defaults still need parity verification.
