...
| Mixed-format paragraphs         | `setShapeParagraphs(shape, [{ align?, runs: [{ text, format? }], endFormat? }])` (several runs per paragraph; `endFormat: { size }` sizes a run-less line, read back with `getParagraphEndFormat`)                                  |
